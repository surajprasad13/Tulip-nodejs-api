...
export async function ingestRecipeImages() {
    console.log("ingestImages");
    
    try {

		const recipes = await RecipesMaster.findAll({
			raw: true,
		})

		for(const recipe of recipes) {
			await ingestImageAndUpdateDB(recipe.id, recipe.image_name)
			await wait1Second()
		}
	} catch (error) {
		console.log(error)
...
async function ingestImageAndUpdateDB(id: number, imageFileName: string) {
	try {
		...
		await RecipesMaster.update({ image_url: imageUrl }, { where: { id: id } })
...
async function ingestImage(
	...
	try {
		...
		if (file) {
			...
			const data: any = await saveFile(`recipes/${id}_${imageFileName}`, fileResized)
